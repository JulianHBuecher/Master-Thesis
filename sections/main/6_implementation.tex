...
Furthermore, to enable access through client applications, the implemented functionality is mapped to specific endpoints, representing resources according to the underlying \acrshort{rest} principles described in Subsection~\ref{ch:Fundamentals:sec:Data Exchange:ssec:REST}.
...
As introduced in the previous section with reference to the class diagram in Figure~\ref{fig:reservation-class-diagram} for the reservation type, the entity, also known as a type, can be located within the \texttt{types} package.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{resources/images/main/6_implementation/PackageDiagram.png}
