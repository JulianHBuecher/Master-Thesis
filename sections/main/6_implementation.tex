...
Therefore, these interfaces, along with the \texttt{CreatedUpdatedProps}, could be categorized as administrative properties to provide uniform handling of system entities internally.
...
Therefore different views out of the context of software architecture are used, to facilitate interested readers of this work to come up with an own implementation utilizing the provided design approaches.
...
Such a flawed design could allow the cancellation of reservations made by other users who have the same \acrshort{id} as the existing reservation at the station, which should be prevented.  Therefore, the implementation in the proposed system approach checks for this rare but possible case.
Regarding future work, this behavior should be observed, as it is still included in the latest \acrshort{ocpp} version 2.0 specification.
...
In addition to task activation and ensuring the specified sequential order, this file also allows for specifying the execution intervals in the form of \texttt{*/1 * * * *}, employed as the syntax for \gls{cron} jobs in the \gls{unix} world, to schedule job processing at specific times.
...
Therefore, the code loads all the entries for the given period and all the \acrshortpl{evse} registered in the system in order to perform an intersection of the two sets and select only the stations inside the \textbf{blue area}, as shown in the diagram below. These stations comprise the subset of options to be offered by the input fields.
...
Therefore, the \textit{Site} scope exists, enabling the assigned role to access all data related to the particular site.
...
\noindent Afterwards, the assignment decisions regarding the \textit{Delete Reservation} and \textit{Enable Reservations} actions are revisited for a better understanding. The choice to allow only the \textit{Admin} user to perform the \textit{Delete Reservation} action is not solely based on the use case design.
